        ...
        while (maxHeap.size() > 0) {
            ...
            // we check if the stack is not empty, 
                ...
                // We check for stack size here because
                    // A-2 B-1 tempN=2  after first iteration ans = 3  i.e. AB_  , tempN=0 ,   'stack contains A-1'
                    // A-1 tempN=2  -> AB_A and tempN=1  but stack is empty if we add this tempN then ans will be AB_A_
                        // we dont want that unncessary _ at the end so we skip add this tempN 
            int tempN = n + 1; 

            while (tempN > 0 && maxHeap.size() > 0) {
                Pair peek = maxHeap.remove();
                // now reduce the peek frequency value by 1 and add it in stack if freq is greater than 0
            ...
            // if tempN is not 0 and stack size is not 0 means there will be more items appending in answer so we need these
            // tempN spaces before adding new charaters to ans.
            if(tempN > 0 && stack.size() > 0) ans += tempN;
    ...
    public static int leastInterval(char[] tasks, int n) {
        ...
        while (maxHeap.size() > 0) {
            ...
            while (tempN > 0 && maxHeap.size() > 0) {
                Pair peek = maxHeap.remove();
                ans += 1;
                System.out.println(peek.ch + " " + peek.freq + " - " + tempN + " " + ans);
                tempN--;
                peek.freq = peek.freq - 1;
                if (peek.freq > 0) {
                    stack.push(peek);
            ...
            while (stack.size() > 0) {
                System.out.println(stack.peek().ch + " " + stack.peek().freq + " - " + tempN + " " + ans + " " + str);
                maxHeap.add(stack.pop());
            }
            System.out.println("******************\n");
